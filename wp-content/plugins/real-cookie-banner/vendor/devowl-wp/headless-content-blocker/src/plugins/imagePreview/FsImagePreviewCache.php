...
abstract class FsImagePreviewCache extends
    ...
    public function set($thumbnails) {
        ...
        foreach ($thumbnails as $embedUrl => $thumbnail) {
            ...
            if ($ch !== \false) {
                $handles[$embedUrl] = ['file' => $file, 'ch' => $ch];
            ...
            if (!empty($alternativeThumbnailUrl)) {
                ...
                if ($ch !== \false) {
                    $handles[$embedUrl . '-alt'] = ['file' => $file, 'ch' => $ch];
                    \curl_multi_add_handle($mh, $ch);
                }
            }
        }
        ...
        foreach ($handles as $embedUrl => $handle) {
            ...
            if ($this->setBySingleResult($handle['ch'], $handle['file'], $thumbnail)) {
            ...
            if (
                $alternativeHandle !== null &&
                $this->setBySingleResult($alternativeHandle['ch'], $alternativeHandle['file'], $thumbnail)
            ) {
                continue;
            }
     ...
     */
    protected function setBySingleResult($ch, $file, $thumbnail) {
